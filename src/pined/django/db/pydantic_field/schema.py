...
app's migrations, mapping a schema hash to the JSON schema it was
generated from. `AlterPydantic` migrations reference these hashes so old
and new versions of a Pydantic model can both be reconstructed later,
e.g. to migrate data between them.
"""
...
class SchemaManager:
    ...
    def generate_model_hash(model: type[pydantic.BaseModel]) -> tuple[str, dict[str, Any]]:
        """
        Hash `model`'s JSON schema, returning `(hash, schema)`.

        The hash is a stable fingerprint of the schema's shape: two
        models with the same fields and types hash the same, regardless

...
from pined.django.settings import DjangoSettings, mixins
...
def build(logs_root: pathlib.Path | None = None, **fields: Any) -> dict[str, Any]:
    """
...
def test_the_whole_config(tmp_path: pathlib.Path) -> None:
    ...
    Building the config also creates the directory, since `dictConfig`
    opens every file as it goes.
    """
